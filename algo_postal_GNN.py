...
class Alg_Postal_GNN(Alg):
    def set_algo(self,model_dir,start_epoch):
        self.model = Postal_GNN(11,6,64,128,64).to(self.device)
...
class Postal_GNN(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        x = self.fc1( torch.cat((x[target_node_idx], apart_feature.view(-1, 6)), dim=1))
        x = self.bn1(x)
        x = self.act1(x)
