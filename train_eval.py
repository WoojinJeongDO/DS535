...
class Train_Eval:
    ...
    def prepare(self,algs,pid,model_dir,start_epoch):
        ...
        if dataset_exists(train_path) & dataset_exists(val_path) & dataset_exists(test_path):
            ...
            for df in [df_traffic, df_amenity, df_education]:
            ...
            X = transaction_data[['trans_date','year_month', 'road_addr','addr','postal_code','build_num','floor',
                                ...
                                'tube_cnt_1km','busstop_cnt_1km','util_cnt_2km','amenity_cnt_2km','park_cnt_2km',
    ...
    def prepare_dataset(self,data_x,data_y):
        ...
        apart_features =  data_x[['floor','trans_type_null','trans_type_indirect','trans_type_direct','area','build_age']].values
        apart_poi = data_x[['tube_cnt_1km','busstop_cnt_1km','util_cnt_2km','amenity_cnt_2km','park_cnt_2km','school_cnt_5km','childcare_cnt_5km']].values
        dataset = []
        for i in tqdm(range(len(data_x)), total=len(data_x), leave=True):
            ...
            data_item.apart_poi = torch.tensor(apart_poi[i], dtype=torch.float)
            ...
            data_item.trans_poi  = torch.cat([data_item.apart_feature , data_item.apart_poi, one_hot_temp], dim=0)
    ...
    def train(self, algs, pid, model_dir):
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for alg in algs:
                ...
                val_targets = []
                alg.train()
                for batch in tqdm(self.train_loader, desc=f"Algo: {alg.name}, Train Epoch {epoch + 1}/{num_epochs}", total=len(self.train_loader.dataset) // self.batch_size):
                    batch = batch.to(self.device)
                    alg.optimizer.zero_grad()
                    output = alg(batch)
                    loss = alg.criterion(output.view(-1), batch.y)
                    alg.optimizer.step()
                ...
                alg.eval()
                with torch.no_grad():
                    for batch in tqdm(self.val_loader, desc=f"Algo: {alg.name}, Validation Epoch {epoch + 1}/{num_epochs}", total=len(self.val_loader.dataset) // self.batch_size):
                        batch = batch.to(self.device)
                        alg.optimizer.zero_grad()
                        output = alg(batch)
                        loss = alg.criterion(output.view(-1), batch.y)
    ...
    def eval(self,algs,model_dir):
        ...
        for alg in algs:
            ...
            print(f'Algo: {alg.name} MSE: {mse:.2f}, MAPE: {mape:.2f}, Pearson Coefficient: {pearson_coeff:.4f}')
    
        results_df = pd.DataFrame(results)
        results_df.to_csv(f"results/{model_dir}.csv", index=False)
        
        metrics = ['MSE', 'MAPE', 'Pearson Coefficient']
