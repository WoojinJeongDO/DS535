...
class Alg_postal_temporal_GIN_att(Alg):
    def set_algo(self,model_dir,start_epoch):
        self.model = Postal_temporal_GIN_att(12,10,64,128,64,11,5611,self.device).to(self.device)
...
class Postal_temporal_GIN_att(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        for i in range(self.num_time_slot):
            ...
            region_embeddings[i] = torch.relu(x)
        region_embeddings = self.attention(region_embeddings.permute(1, 0, 2)).sum(dim=1)
